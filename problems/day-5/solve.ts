    ...
    parseInputData(data: string): InType {
        ...
        data.split('\n').forEach(line => {
            ...
            if (line.includes('-')) {
                ...
                ingredientsData.fresh.push([parseInt(freshInterval[0]), parseInt(freshInterval[1])])
                return;
            }
...
async function solveFirst(version: string) {
    ...
    data.ingredients.forEach((ingredient) => {
        const isFresh = data.fresh.find((interval) => interval[0] <= ingredient && ingredient <= interval[1])
        if (isFresh) {
            freshIngredients += 1
        }
    })
    ...
    while (changed) {
        ...
        for (let i = 0; i < intervals.length - 1; i++) {
            ...
            for (let j = i + 1; j < intervals.length; j++) {
                ...
                if (secondInterval[0] <= firstInterval[0] && secondInterval[1] >= firstInterval[0] && secondInterval[1] <= secondInterval[1]) {
                    firstInterval[0] = secondInterval[0];
                    intervals[j] = null;
                    changed = true;
                    continue;
                }
                if (secondInterval[0] >= firstInterval[0] && secondInterval[0] <= firstInterval[1] && secondInterval[1] >= firstInterval[1]) {
